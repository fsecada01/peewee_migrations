...
def init(ctx):
    ...
    if os.path.exists(params["config"]):
        click.secho(
            "Configuration file %r already exists." % params["config"], fg="red"
        )
        sys.exit(2)
...
def load_conf(ctx):
    ...
    for path in conf["models"]:
        for model in import_models(path):
            ...
            if db is None:
                click.secho(
                    "Database is not specified for model %s." % model.__name__, fg="red"
                )
                sys.exit(3)
            databases.add(db)
            models.append(model)
    ...
    if len(databases) != 1:
        click.secho(
            "%r: found multiple database instances, only one is supported." % filename,
            fg="red",
        )
        sys.exit(3)
...
def watch(ctx, traceback, tracecode, serialize):
    ...
    try:
        ...
        if tracecode and context.get("code"):
            ...
            code = [
                (
                    click.style("% 5d " % num, fg="yellow")
                    + click.style(line, fg="white")
                )
    ...
    if not result:
        ...
        for line, text in result[1:]:
            text = "".join(
                [
                    click.style(j, "magenta" if i % 2 else "yellow")
...
@click.option(
    "-a", "--autocommit", default=False, is_flag=True, help="run without a transaction"
)
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="ignore errors and try to continue",
)
@click.option("--traceback", is_flag=True, help="Show traceback")
@click.pass_context
def migrate(ctx, to, fake, skip, autocommit, traceback, ignore_errors):
    ...
    for step in steps:
        ...
        if step.direction == "forward":
            ...
            click.secho("[ ] " + step.name + serialized, fg="yellow")
        try:
            for descr, color, skipped in step.run(
                fake=fake, skip=skip, ignore_errors=ignore_errors
            ):
                if skipped:
                    prefix = "[ ]"
                else:
