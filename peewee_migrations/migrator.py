        ...
        if hasattr(field, "deconstruct"):
            ...
            for cls in type(field).__mro__:
                if cls in self._deconstructors:
                    self._deconstructors[cls](
                        field, params=params, complete=complete
                    )
                    break
...
def deconstruct_foreignkey(field, params, complete):
    ...
    if (
        complete
        or field.rel_field.name != field.rel_model._meta.primary_key.name
    ):
        params["field"] = field.rel_field.name
    if complete or (
        field.backref and field.backref != field.model._meta.name + "_set"
    ):
...
def deconstruct_deferredforeignkey(field, **_):
    raise TypeError(
        "DeferredForeignKey '%s.%s' will not be resolved, use ForeignKeyField instead."
...
class Orm:
    def __init__(self, models):
        ...
        self._mapping = {model._meta.name: model for model in models}
    ...
    def ForeignKeyField(self, model, **kwargs):
        ...
        if model in self.mapping:
            ...
            for m in self.models:
                ...
                raise TypeError(
                    'Model "%s" is used, but not included to the watch list.'
                    % model
                )
...
class NodeDeconstructor:
    ...
    def code_Column(self, node):
        return CallBlock(
            "Column", args=(self.get_code(node.source), node.name)
        )
    ...
    def model_to_code(self, model):
        ...
        if model._meta.primary_key is not None:
            if isinstance(model._meta.primary_key, peewee.CompositeKey):
                names = ", ".join(
                    repr(x) for x in model._meta.primary_key.field_names
                )
                meta.append(
                    INDENT + "primary_key = peewee.CompositeKey(%s)" % names
                )
...
class Storage:
    def __init__(
        self, database, models, migrate_table="migratehistory", **kwargs
    ):
        self.database = database
        self.models = models
    ...
    def done(self):
        return [
            x.name
            for x in self.history_model.select().order_by(
                self.history_model.id
            )
        ]
    ...
    def set_undone(self, name):
        self.history_model.delete().where(
            self.history_model.name == name
        ).execute()
    ...
    def get_steps(self, name):
        ...
        if name == "zero":
            ...
            direction = "forward"
            steps = (["zero"] + self.done)[-1:] + self.undone[
                : self.undone.index(name) + 1
            ]
...
class FileStorage(Storage):
    ...
    def todo(self):
        ...
        todo = list(
            sorted(
                f[:-3]
                for f in os.listdir(self.migrate_dir)
...
class Router:
    def __init__(
        self, database, models, storage_class=FileStorage, **storage_kwargs
    ):
        self.database = database
        self.models = models
        self.storage = storage_class(
            database=database, models=models, **storage_kwargs
        )
        ...
        for name_from, name_to in steps:
            ...
            if direction == "forward":
                migrator.add_operation(
                    self.storage.set_done, args=(step_name,), forced=True
                )
            else:
                migrator.add_operation(
                    self.storage.set_undone, args=(step_name,), forced=True
                )
...
class Migrator:
    ...
    def __init__(
        ...
        *,
        ...
        if run_data_migration:
            data_migration = lambda: run_data_migration(
                old_orm=self.old_orm, new_orm=self.new_orm
            )
        else:
            data_migration = None

        self.op = get_operations_for_db(
            database, atomic=atomic, data_migration=data_migration
        )

        if serialize:
            self.recorder = OperationSerializer(
                self.op,
                self.old_orm,
                self.new_orm,
                self.serialized.add_operation,
            )
        else:
    ...
    def add_operation(
        self, obj, *, args=None, kwargs=None, color=None, forced=False
    ):
    ...
    def migrate(self):
        if self.run_serialized:
            return self.run_serialized(
                op=self.recorder, old_orm=self.old_orm, new_orm=self.new_orm
            )
        ...
        for name in [m for m in models2 if m not in models1]:
            for field in models2[name]._meta.sorted_fields:
                if (
                    isinstance(field, peewee.ForeignKeyField)
                    and field.deferred
                ):
    ...
    def _render_migrate_state(self, model1, model2):
        ...
        return State(
            ...
            drop_indexes=[
                indexes1[key] for key in set(indexes1) - set(indexes2)
            ],
            add_indexes=[
                indexes2[key] for key in set(indexes2) - set(indexes1)
            ],
            drop_constraints=[
                constraints1[key]
                for key in set(constraints1) - set(constraints2)
            ],
            add_constraints=[
                constraints2[key]
                for key in set(constraints2) - set(constraints1)
            ],
            drop_fields=[fields1[key] for key in set(fields1) - set(fields2)],
            add_fields=[fields2[key] for key in set(fields2) - set(fields1)],
            check_fields=[
                (fields1[key], fields2[key])
    ...
    def add_data_migration_hints(self, field1, field2, type_changed):
        ...
        def run_helper(helper_func, old_field, new_field, code):
            kwargs = {
                ...
                "old_field": old_field,
                "new_field": new_field,
                "old_model": ("old_" + old_field.model._meta.name)
                if old_field
                else None,
            ...
            if kwargs["old_model"]:
                code.set_var_if_not_exists(
                    kwargs["old_model"],
                    "old_orm[%r]" % old_field.model._meta.name,
                )
            if kwargs["new_model"]:
                code.set_var_if_not_exists(
                    kwargs["new_model"], "new_orm[%r]" % kwargs["new_model"]
                )
            code.add_operation(result)

        if (field1 is None or field1.null) and (
            field2 is not None and not field2.null
        ):
            run_helper(set_not_null_helper, None, field2, self.forward_hints)
        if (field2 is None or field2.null) and (
            field1 is not None and not field1.null
        ):
        ...
        if type_changed:
            field_check1 = (
                field1.rel_field
                if isinstance(field1, peewee.ForeignKeyField)
                else field1
            )
            field_check2 = (
                field2.rel_field
                if isinstance(field2, peewee.ForeignKeyField)
                else field2
            )
            for (tp1, tp2), helper in DATA_MIGRATE_HINTS:
                if isinstance(field_check1, tp1) and isinstance(
                    field_check2, tp2
                ):
                    run_helper(helper, field1, field2, self.forward_hints)
                    break
            for (tp1, tp2), helper in DATA_MIGRATE_HINTS:
                if isinstance(field_check2, tp1) and isinstance(
                    field_check1, tp2
                ):
...
class MigrateCode:
    ...
    def add_operation(self, result):
        ...
        if alerts:
            self.alerts.extend(
                [(len(self.results), alert) for alert in alerts]
            )
        if comment:
...
def set_not_null_helper(new_field, **_):
    ...
    if default is None:
        ...
        return (
            "Apply default value %s to the field {new_model}.{new_field.name}"
            % escaped_repr(default),
            "{new_model}.update({{{new_model}.{new_field.name}: %s}})"
            ".where({new_model}.{new_field.name}.is_null(True))"
            % escaped_repr(default),
        )
...
class OperationSerializer:
    ...
    def __getattr__(self, attr):
        ...
        def recorder(*args, **kwargs):
            ...
            s_kwargs = [
                "%s=%s" % (k, self.__serialize(v)) for k, v in kwargs.items()
            ]
            self.recorder(
                (
                    "",
                    "op.%s(%s)" % (obj.__name__, ", ".join(s_args + s_kwargs)),
                )
            )
...
class Operations:
    ...
    def alter_table(self, model):
        return self.ctx().literal("ALTER TABLE ").sql(model)
    ...
    def add_column(self, field):
        ...
        return ctx.literal(" ADD COLUMN ").sql(field.ddl(ctx))
    ...
    def drop_column(self, field):
        return (
            self.alter_table(field.model).literal(" DROP COLUMN ").sql(field)
        )

    @operation
    def rename_column(self, field, column_name):
        return self._rename_column(
            field.model, field, field.column_name, column_name
        )
...
class PostgresqlOperations(Operations):
    ...
    def _drop_primary_key_constraint(self, model):
        ...
        return (
            ...
            .sql(
                lambda: self._get_primary_key_constraint_name(
                    table_name, schema
                )
            )
        )
    ...
    def _add_foreign_key_constraint(self, model, field):
        ...
        return [
            super()
            ._add_foreign_key_constraint(model, field)
            .literal(" NOT VALID"),
            lazy_query()
            .literal("ALTER TABLE ")
            .sql(peewee.Entity(table_name))
            ...
            .sql(
                lambda: self._get_foreign_key_constraint_name(
                    table_name,
                    schema,
                    column_name,
                    rel_table_name,
...
class MySQLOperations(Operations):
    ...
    def drop_index(self, model, index):
        return (
            model._schema._drop_index(index, safe=False)
            .literal(" ON ")
            .sql(model)
        )
    ...
    def _add_not_null(self, model, field):
        ctx = self.alter_table(model)
        return ctx.literal(" MODIFY ").sql(field.ddl(ctx))
    ...
    def _get_foreign_key_constraint_name(
        self, table_name, column_name, rel_table_name, rel_column_name
    ):
        sql = (
        ...
        cursor = self._database.execute_sql(
            sql, (table_name, column_name, rel_table_name, rel_column_name)
        )
        result = cursor.fetchall()
        return peewee.Entity(result[0][0] if result else "?")
