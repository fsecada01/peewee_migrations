...
import unittest
from peewee import *
from apistar_peewee.migrator import (
    Migrator,
    Column,
    Router,
    MemoryStorage,
...
class Metadata(BaseMetadata):
    ...
    def get_columns(self, table, schema=None):
        ...
        for name, modifiers in self.get_column_type_modifiers(table, schema):
            columns[name].field_class = make_field_class(
                columns[name].field_class, modifiers
            )
        return columns
...
class MigrationTestCase(unittest.TestCase):
    ...
    def assertModelsEqual(self, model1, model2):
        ...
        indexes1 = sorted(
            [(tuple(names), unique) for names, unique in model1._meta.indexes]
        )
        indexes2 = sorted(
            [(tuple(names), unique) for names, unique in model2._meta.indexes]
        )
        self.assertEqual(indexes1, indexes2)
        exclude = ["backref", "default", "on_delete", "on_update"]
        fields1 = {
            k: (type(v), Column(v).to_params(exclude))
            for k, v in model1._meta.fields.items()
        }
        fields2 = {
            k: (type(v), Column(v).to_params(exclude))
            for k, v in model2._meta.fields.items()
        }
        if PRINT_DEBUG:
